...
export default function AdminProductSubmissionsPage() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const data = await fetchAPI(`/product-submissions?status=${encodeURIComponent(statusFilter)}`, {
        headers: {
  ...
  const approve = async (id: string) => {
    ...
    try {
      await fetchAPI(`/product-submissions/${id}/approve`, {
        method: 'PATCH',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
  ...
  const reject = async (id: string) => {
    ...
    setActionLoadingId(id);
    try {
      await fetchAPI(`/product-submissions/${id}/reject`, {
        method: 'PATCH',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        },
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-white">Semakan Produk Vendor</h1>
      ...
      <div className="bg-zinc-900 border border-white/10 rounded-xl overflow-hidden">
        <table className="w-full text-left text-sm text-gray-400">
          ...
          <tbody className="divide-y divide-white/10">
            {loading ? (
              <tr>
                <td colSpan={7} className="px-6 py-8 text-center">Memuatkan...</td>
              </tr>
            ) : items.length === 0 ? (
              <tr>
                <td colSpan={7} className="px-6 py-8 text-center">Tiada submission.</td>
              </tr>
            ) : (
              items.map((it) => (
                <tr key={it.id} className="hover:bg-white/5 transition-colors">
                  <td className="px-6 py-4">
                    <div>
                      <p className="text-white font-medium">{it.vendor?.name || '-'}</p>
                      <p className="text-xs">{it.vendor?.email || '-'}</p>
                    </div>
                  </td>
